...
def validate_hallucination_guardrail(
    ...
    if root_cause == "UNKNOWN":
        # If the root cause was UNKNOWN in diagnosis, planner/replan shouldn't declare a definitively known hardware defect
        if any(term in text_lower for term in ["confirmed hardware defect", "defective optical laser", "corrupt bootloader"]):
            violations.append(
    ...
    if "bill" in complaint_text and not any(k in complaint_text for k in ["cable", "cut", "fiber", "line", "down"]):
        if any(k in text_lower for k in ["severed fiber cable", "underground cable cut", "optical line break"]):
            violations.append(
