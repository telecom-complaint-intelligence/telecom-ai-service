...
from agents.high.agents.diagnosis import diagnosis_agent
from agents.high.agents.policy import policy_agent
from agents.high.agents.risk import risk_agent
from agents.high.agents.planner import planner_agent
from agents.high.agents.critic import critic_agent
from agents.high.agents.replan import replan_agent
...
def finalize_node(state: ComplaintState):
    ...
    # Map final decision to CRITICAL_DISPATCH if it's CRITICAL and it's an infrastructure/dispatch case
    if str(final_dec).strip().upper() == "CRITICAL":
        txt = str(state.get("complaint_text", "")).lower()
        if "tower" in txt or "lightning" in txt or "excavator" in txt or "severed" in txt or "911" in txt or "psap" in txt or "5000" in txt or state.get("scope") == "area":
            final_dec = "CRITICAL_DISPATCH"

    # Evaluate whether case was moved to CRITICAL
    moved_to_crit = (
        str(final_prio).strip().upper() == "CRITICAL" or
        str(final_dec).strip().upper() in ["CRITICAL", "CRITICAL_DISPATCH"]
    )
